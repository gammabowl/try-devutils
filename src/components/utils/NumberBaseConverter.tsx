...
import { Binary, AlertCircle, ArrowDown, RotateCcw, BookOpen } from "lucide-react";
                ...
                {examples.map((example, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-muted/50 rounded-md gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="font-mono text-sm text-foreground">{bases[example.base].prefix}{example.value}</div>
                      <div className="text-xs text-muted-foreground">{example.desc}</div>
                    </div>
                    <Button onClick={() => { setFromBase(example.base); setInput(example.value); }} variant="outline" size="sm" className="h-7 text-xs flex-shrink-0">Use</Button>
                  </div>
                ))}
              </div>
            </PopoverContent>
